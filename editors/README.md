...
`yamlls` speaks LSP 3.16 over stdio, so every editor here is a thin client that
launches the binary and forwards `yamlls.*` settings as `initializationOptions`
...
[GitHub releases](https://github.com/home-operations/yamlls/releases), selecting
the asset for the host OS/arch:

```
yamlls_{version}_{os}_{arch}.tar.gz   # linux, darwin
yamlls_{version}_{os}_{arch}.zip      # windows
```

| Editor | Language | Subdir | Binary source |
| ------ | -------- | ------ | ------------- |
| VS Code | TypeScript (`vscode-languageclient`) | [`vscode/`](./vscode) | downloaded to global storage; override with `yamlls.path` |
